  ...
  if (allInvitados.length === 0) {
    ...
    try {
      const querySnapshot = await db
        .collection("invitadosXVDannaMichael2026")
...
async function agregarInvitado(e) {
  ...
  try {
    const docRef = await db.collection("invitadosXVDannaMichael2026").add({
...
async function eliminarInvitado(id) {
  ...
  if (confirmar) {
    mostrarCargando(true);
    try {
      await db.collection("invitadosXVDannaMichael2026").doc(id).delete();
...
//       await db.collection("invitadosXVDannaMichael2026").add({
